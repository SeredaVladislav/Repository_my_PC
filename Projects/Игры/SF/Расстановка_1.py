...
def func_lock(input_coords, x, y):
    for i in input_coords:
        if i not in reserved_cells:
            ...
            res_cell_x1 = str(x) + str(y - 1)
            res_cell_x2 = str(x) + str(y + 1)
            # ---------------------------
            # Крайние координаты (по вертикали (y)) от выстрела (для запрета)
            res_cell_y1 = str(x - 1) + str(y)
            res_cell_y2 = str(x + 1) + str(y)
            # ---------------------------

            all_res_cells = [res_cell_x1, res_cell_x2, res_cell_y1, res_cell_y2]
